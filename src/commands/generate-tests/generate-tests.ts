...
export const generateTests = async (llmService: LlmApiService): Promise<void> => {
    ...
    const testFileName = path.basename(fileName, path.extname(fileName)) + TEST_FILE_SUFFIX;
    ...
    const loadingMessage = vscode.window.withProgress({
        ...
        try {
            ...
            let result: string | null = await llmService.generate(prompt);
            ...
            try {
                ...
                testDocument = await vscode.workspace.openTextDocument(vscode.Uri.parse(`untitled:${testFilePath}`));
            }
            ...
            if (testDocument.getText().length === 0) {
                ...
                edit.insert(testFileUri, testDocument.positionAt(testDocument.getText().length), `\n\n${result}`);
            }
